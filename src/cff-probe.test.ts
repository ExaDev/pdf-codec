...
// The real, vendored STIX Two Math font's own 'CFF ' table is genuine CFF data -- 691 KB of it, produced by a real font toolchain, not a fixture written to satisfy this parser -- so the header/INDEX/DICT walk is exercised against a real file first, and the hand-built fixtures below only cover the shapes that font does not happen to contain (a CID-keyed Top DICT, and the malformed cases).
function stixCffBytes(): Uint8Array<ArrayBuffer> {
...
// A CFF INDEX (spec section 5), with offSize 1 -- every fixture here is small enough for one-byte offsets, and the real font above already covers a larger offSize (its own Top DICT INDEX uses 3).
function cffIndex(entries: readonly (readonly number[])[]): number[] {
  if (entries.length === 0) {
    return [0, 0];
  }
  const offsets = [1];
...
const CFF_HEADER = [1, 0, 4, 1]; // major 1, minor 0, hdrSize 4, offSize 1

function cffFont(name: string, topDict: readonly number[], header: readonly number[] = CFF_HEADER): Uint8Array<ArrayBuffer> {
...
const ROS_OPERANDS_AND_OPERATOR = [28, 0x01, 0x87, 28, 0x01, 0x88, 139, 12, 30];
...
describe('probeCff against the real vendored STIX Two Math CFF table', () => {
  it('reads the header, Name INDEX, and Top DICT of a genuine 691 KB CFF program', () => {
    const probe = probeCff(stixCffBytes());
  ...
  it('reports the font as NOT CID-keyed, which is what makes CID == GID safe for it', () => {
    ...
    expect(probeCff(stixCffBytes())?.cidKeyed).toBe(false);
  });
});
