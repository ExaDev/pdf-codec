...
// A composite may reference another composite. The format sets no explicit limit, but real fonts nest one level (a base letter plus a mark) and occasionally two; this bounds the walk so a font whose components reference each other in a cycle terminates rather than recursing until the stack gives out.
...
export function parseGlyf(font: SfntFont, options: GlyfOptions): GlyfTable | undefined {
  ...
  const resolveInkBounds = (glyphId: number, depth: number): GlyphInkBounds | undefined => {
    ...
    let bounds: GlyphInkBounds | undefined;
    for (const component of components) {
      ...
      if (componentBytes === undefined) {
        return undefined; // an unreadable component, as opposed to a legitimately empty one below
      }
      if (componentBytes.length === 0) {
        continue; // a component that draws nothing contributes no ink to the union
      }
  ...
  return {
    ...
    glyphHeader,
    compositeComponents,
    glyphInkBounds: (glyphId: number) => resolveInkBounds(glyphId, 0),
  };
}
